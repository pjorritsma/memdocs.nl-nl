...
- De BitLocker-herstel service vereist HTTPS voor het versleutelen van de herstel sleutels in het netwerk van de configuratie client beheren op het beheer punt. Er zijn twee opties:
...
- BitLocker-beheer wordt niet ondersteund op virtuele machines (Vm's). Daarom werken sommige functies mogelijk niet zoals verwacht op virtuele machines. Bijvoorbeeld, BitLocker-beheer start de versleuteling niet op vaste schijven van virtuele machines. Extra harde schijven in virtuele machines kunnen worden weer gegeven als compatibel, zelfs als ze niet zijn versleuteld.

...
- **Toepassingstype**: Selecteer het toepassingstype. Uw opties zijn:
  ...
  - **Microsoft Edge-browser toevoegen**: selecteer **Microsoft Edge-browser** en kies het **Edge-kioskmodustype**:
  ...
  - **Kiosk-browser toevoegen**: Selecteer **Browserinstellingen voor de kiosk**. Met deze instellingen beheert u een webbrowser-app op de kiosk. Zorg ervoor dat u de [Kioskbrowser-app](https://businessstore.microsoft.com/store/details/kiosk-browser/9NGB5S5XG2KP) uit de Store ophaalt en deze aan Intune toevoegt als een [client-app](../apps/apps-add.md). Wijs vervolgens de app toe aan de kioskapparaten.
    ...
    - **Toegestane websites**: Gebruik deze instelling om toe te staan dat bepaalde websites worden geopend. Met andere woorden, gebruik deze functie om het aantal websites dat op het apparaat kan worden geopend, te beperken. U kunt bijvoorbeeld toestaan dat alle websites op `http://contoso.com` kunnen worden geopend. Standaard worden alle websites toegestaan.
      ...
      Als u specifieke websites wilt toestaan, moet u een bestand uploaden dat een lijst met toegestane websites op afzonderlijke regels bevat. Als u geen bestand toevoegt, zijn alle websites toegestaan. Intune biedt standaard ondersteuning voor jokertekens. Dus als u het domein invoert, zoals `sharepoint.com`, zorg er dan voor dat subdomeinen automatisch worden toegestaan, zoals `contoso.sharepoint.com`, `my.sharepoint.com`, enzovoort.
 ...
 - **Onderhoudsvenster opgeven voor opnieuw opstarten van apps**: De standaardinstelling is Niet geconfigureerd. Selecteer Vereisen om te controleren op apps die opnieuw moeten worden gestart om de installatie te voltooien.
 
     Als u Kiosk Browser of een andere Microsoft Store voor Bedrijven-app gebruikt, moet u bepalen hoe vaak moet worden gecontroleerd op app-updates waarvoor opnieuw moet worden gestart om de installatie van de toepassing te voltooien. Als Microsoft Store voor Bedrijven-apps niet zijn geconfigureerd, worden ze opnieuw gestart op een niet-gepland tijdstip, drie dagen nadat een app-update is geïnstalleerd.
     
     - **Begintijd van onderhoudsvenster**: Selecteer de datum en tijd van de dag waarop u clients wilt gaan controleren op app-updates waarvoor opnieuw moet worden gestart. De reguliere begintijd is middernacht, oftewel nul minuten.
     
     - **Terugkeerpatroon voor onderhoudsvenster**: De standaardinstelling is dagelijks.
         Stel in hoe vaak onderhoudsvensters voor app-updates moeten worden uitgevoerd. Het wordt aanbevolen om dit dagelijks te doen om te voorkomen dat apps op een niet-gepland tijdstip opnieuw worden gestart.
...
- **Browser en apps**: Voeg de apps toe die u op het apparaat in kioskmodus wilt uitvoeren. U kunt verschillende apps toevoegen.
  ...
  - **Browsers**
    ...
    - **Microsoft Edge toevoegen**: Microsoft Edge wordt toegevoegd aan het app-raster en alle apps kunnen in deze kiosk worden uitgevoerd. Het **Microsoft Edge-kioskmodustype kiezen**:
